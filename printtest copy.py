...
# Compare with threshold
    ...
    def P_eff_func(xin,p1,A1,A2,A3):
        x, sigma = xin        
        return A1*np.exp((-(x-p1)**2)/(2*sigma))
        #return A1*np.exp((-(x-u1)**2)/(1*0.21354))
    # bounds=([0, 0, -np.inf, -np.inf, -np.inf], [1, 1, np.inf, np.inf, np.inf])
    ini_guess = [0.5]*4
    c,cov = curve_fit(P_eff_func, (t_eff_norm, sig_array),P_eff_norm, p0=ini_guess, maxfev=100000)
    print('p1:')
    print(c[0])
    print('A1:')
    print(c[1])
    ...
    def P_fitted_func(x,A1,Ers,Rrs):
        V, Q, t_eff, p1, cond, sigma = x
        return  (Ers*V)+(Rrs*Q)+(cond*A1*np.exp((-(t_eff-p1)**2)/(2*sigma)))
    ...
    #p2_array = np.array(np.full(np.size(P_insp),c[1]))
    ...
    ini_guess2 = [0]*3
    param,cov2 = curve_fit(P_fitted_func,(V_insp, Q_insp, t_eff2, p1_array, cond, sig_array),P_insp, bounds=([-np.inf, 0, 0], [np.inf, np.inf, np.inf]))
...
#    return P, Q, Ers, Rrs, PEEP, PIP, Vtidal, V, A1, A2, A3, u, t_eff2, cond
    ...
    Psim_insp = (Ers*V_insp)+(Rrs*Q_insp)+(cond*A1*np.exp((-(t_eff2-p1_array)**2)/(2*sigma)))
